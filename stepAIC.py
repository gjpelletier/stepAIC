...
def plot_predictions_from_test(model, X, y, scaler='off'):
    ...
    scaler= scaler object used to standardize X in the fitted model
    ...
    fig.suptitle(
        f"Predictions compared with observed data and residuals (RMSE={rmse:.3f})")
    plt.tight_layout()
...
def gbr(X, y, **kwargs):
    ...
    **kwargs (optional keyword arguments):
    ...
    if data['verbose'] == 'on':
        ...
        axs[1].set_title("Residuals vs. Predicted")
        fig.suptitle(
            f"Predictions compared with observed data and residuals (RMSE={stats['RMSE']:.3f})")
        plt.tight_layout()
        # plt.show()
