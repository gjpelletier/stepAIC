...
def elastic(X, y, **kwargs):
    ...
    **kwargs (optional keyword arguments):
        ...
        l1_ratio= Float between 0 and 1 passed to ElasticNet 
            ...
            Note that a good choice of list of values for l1_ratio 
            is often to put more values close to 1 (i.e. Lasso) 
            and less close to 0 (i.e. Ridge), 
            as in [.1, .3, .5, .7, .9, .95, .99, .999, 1] (default).        
        verbose= 'on' (default) or 'off' where
            'on': display model summary on screen 
            'off': turn off display of model summary on screen
    ...
    defaults = {
        ...
        'alpha_max': 1.0e3,
        'n_alpha': 1000,
        'l1_ratio': [.1, .3, .5, .7, .9, .95, .99, .999, 1],      # 0.5 or list [.1, .5, .7, .9, .95, .99, 1]
        'verbose': 'on'
        }
    ...
    # Suppress warnings
    warnings.filterwarnings('ignore')
    print('Fitting ElasticNetCV regression models, please wait ...')
    if data['verbose'] == 'on':
        print("\n")
