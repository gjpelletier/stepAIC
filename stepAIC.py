...
def stepwise(X, y, **kwargs):
    ...
    **kwargs (optional keyword arguments):
    ...
    if ctrl:
        print('Check X: Stewpise can not handle dummies. Try using lasso or ridge if X has dummies.','\n')
        sys.exit()
...
def lasso_novif(X, y, **kwargs):

    """
    Python function for Lasso linear regression 
    using k-fold cross-validation (CV) or to minimize AIC or BIC
    simplified version with no pcov or VIF output
