    ...
    **kwargs (optional keyword arguments):
    ...
    if data['verbose'] == 'on':
        ...
        plt.axvline(lasso.alpha_, linestyle="--", color="black", 
                    label="CV selected alpha={:.2e}".format(model_cv.alpha_))        
        # ymin, ymax = 2300, 3800
        # plt.ylim(ymin, ymax)
        plt.xlabel(r"$\alpha$")
    ...
    if data['verbose'] == 'on':
        ...
        plt.axvline(lasso.alpha_, linestyle="--", color="black", 
                    label="LarsCV selected alpha={:.2e}".format(model_lars_cv.alpha_))
    ...
    if data['verbose'] == 'on':
        ...
        ax.vlines(
            ...
            results["AIC"].min(),
            results["AIC"].max(),
            label="AIC selected alpha={:.2e}".format(model_aic.alpha_),
            linestyles="--",
            color="tab:blue",
        )
        ax.vlines(
            ...
            results["BIC"].min(),
            results["BIC"].max(),
            label="BIC selected alpha={:.2e}".format(model_bic.alpha_),
            linestyle="--",
            color="tab:orange",
        )
    ...
    if data['verbose'] == 'on':
        ...
        plt.figure()
        plt.scatter(y_pred_cv, (res_cv), s=30, label=('LassoCV (RMSE={:.2f})'.format(rmse_cv)))
        plt.scatter(y_pred_lars_cv, (res_lars_cv), s=20, label=('LassoLarsCV (RMSE={:.2f})'.format(rmse_lars_cv)))
