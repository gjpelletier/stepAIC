...
def stacking(X, y, **kwargs):
    ...
    **kwargs (optional keyword arguments):
    ...
    base_regressors = []
    if data['Lasso']=='on':
        base_regressors.append(('Lasso', 
            LassoCV(random_state=data['random_state'])))
    if data['Ridge']=='on':
        base_regressors.append(('Ridge', 
            RidgeCV(alphas=alphas)))
    if data['ElasticNet']=='on':
        base_regressors.append(('ElasticNet', 
            ElasticNetCV(l1_ratio=data['l1_ratio'],random_state=data['random_state'])))
    if data['SGD']=='on':
        base_regressors.append(('SGD', 
            SGDRegressor(random_state=data['random_state'])))
    if data['KNN']=='on':
        base_regressors.append(('KNN', 
            KNeighborsRegressor()))
    if data['SVR']=='on':
        base_regressors.append(('SVR', 
            SVR(kernel='rbf')))
    if data['MLP']=='on':
        base_regressors.append(('MLP', 
            MLPRegressor(random_state=data['random_state'])))
    if data['XGB']=='on':
        base_regressors.append(('XGB', 
            GradientBoostingRegressor(random_state=data['random_state'])))
    if data['DecisionTree']=='on':
        base_regressors.append(('DecisionTree', 
            DecisionTreeRegressor(random_state=data['random_state'])))
    if data['RandomForest']=='on':
        base_regressors.append(('RandomForest', 
            RandomForestRegressor(n_estimators=50, random_state=data['random_state'])))
    ...
    strength.set_index('Coefficient',inplace=True)
    model_outputs['strength'] = strength
    print("Strength coefficients of base_regressors in model_outputs['strength']:")
    print("\n")
    print('- high positive intercept suggests base models under-predict target')
    print('- low negative intercept suggests base models over-predict target')
    ...
    print('- try turning off base_regressors with lowest negative coefficients')
    print("\n")
    print(model_outputs['strength'].to_markdown(index=True))
    print("\n")
