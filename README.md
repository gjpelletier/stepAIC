...
The stepAIC algorothim has the option to use either a forward (default), backward, or bi-directional stepwise selection for the optimum set of predictor variables as follows:
...
- Bi-directional selection starts with forward selection and proceeds until the selected  best features minimize the AIC or BIC. Then it switches to backward selection and removes selected features one at a time until there is no further improvement in the model.
