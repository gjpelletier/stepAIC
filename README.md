...
Note that the model_objects and model_outputs returned by the lasso function also contain the best-fit sklearn model objects and many other useful outputs as described by help(lasso). All of the optional arguments are also explained by running help(lasso) 
...
Also note that the best_model and best_features returned by the lasso function also provide the best-fitted statsmodels model object and a list of the selected features of the best-fit model. All of the optional arguments are also explained by running help(stepwise) 
